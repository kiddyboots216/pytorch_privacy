...
class DPQuery(object):
    ...
    def preprocess_record(self, params, record):
        ...
        separate session, or even on a different machine, so it should not depend
        on any inputs other than those provided as input arguments. In
        particular, implementations should avoid accessing any tensors or
...
class SumAggregationDPQuery(DPQuery):
    ...
    def accumulate_preprocessed_record(self, sample_state, preprocessed_record):
        return torch.add(sample_state, preprocessed_record)

    def merge_sample_states(self, sample_state_1, sample_state_2):
        return torch.add(sample_state_1, sample_state_2)
